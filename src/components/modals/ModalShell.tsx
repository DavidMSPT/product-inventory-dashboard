...
export default function ModalShell({ children, onClose }: { children: React.ReactNode; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50">
      <div onClick={onClose} className="absolute inset-0 bg-black/30 opacity-0 animate-[fadeIn_150ms_ease-out_forwards]" />
      <div className="absolute inset-0 flex items-end justify-center p-4 sm:items-center">
        <div className="w-full max-w-lg scale-95 opacity-0 animate-[popIn_180ms_ease-out_forwards]">{children}</div>
      </div>
