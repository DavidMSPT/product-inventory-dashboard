...
import { setThemePersist, setViewPersist } from './state'
...
export function reducer(state: GlobalState, action: Action): GlobalState {
  switch (action.type) {
    ...
    case 'SET_VIEW': setViewPersist(action.view); return { ...state, view: action.view }
    ...
    case 'SET_THEME': setThemePersist(action.theme); return { ...state, theme: action.theme }
